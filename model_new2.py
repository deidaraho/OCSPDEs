...
( n_f, n_t, n_u, n_p,
  ...
  l1, l2, n1, n2, n3, m,
  g_vector, g2_vector,
  lut, nuu ) = retrieve_data("model_new2_lin.data", "model_new2_lut.data",
                                 "model_new2_nuu.data")

tq1_drt = './target_area/tq_point08.npy'
tq2_drt = './target_area/tq_point09.npy'
tq3_drt = './target_area/tq_point14.npy'

part = '(whole_4)'
tq_point = np.load( tq2_drt )
tq_point2 = np.load( tq2_drt )
tq_point3 = np.load( tq2_drt )
...
def objf(out, x):
    out[0] = 0.5/n_f * ( 1000.0 * np.sum( ( x[tidx] - 4.5*z_d ) * ( x[tidx] - 4.5*z_d ) )
...
def objg(out, x):
    out[tidx] = 1000.0/(n_f) * ( x[tidx] - 4.5*z_d )
...
def objf2_local(out, x):
    out[0] = 0.5/n_f * ( 10.0/num_t * np.sum( ( x[tidx[0:n_f/3,tqidx]] - z_d ) * ( x[tidx[0:n_f/3,tqidx]] - z_d ) ) +
                         10.0/num_t * np.sum( ( x[tidx[n_f/3:2*n_f/3,tq2idx]] - z_d ) * ( x[tidx[n_f/3:2*n_f/3,tq2idx]] - z_d ) ) +
                         10.0/num_t * np.sum( ( x[tidx[2*n_f/3:,tq3idx]] - z_d ) * ( x[tidx[2*n_f/3:,tq3idx]] - z_d ) ) +
                         + t_final * ( alpha * x[vidx].dot( x[vidx] ) +
                                       alpha * x[v2idx].dot( x[v2idx] ) +
                                       beta * x[vuidx].dot( x[vuidx] ) +
                                       beta * x[v2uidx].dot( x[v2uidx] ) +
...
def objg2_local(out, x):
    out[tidx[0:n_f/3,tqidx]] = 10.0/(n_f*num_t) * ( x[tidx[0:n_f/3,tqidx]] - z_d )
    out[tidx[n_f/3:2*n_f/3,tq2idx]] = 10.0/(n_f*num_t) * ( x[tidx[n_f/3:2*n_f/3,tq2idx]] - z_d )
    out[tidx[2*n_f/3:,tq3idx]] = 10.0/(n_f*num_t) * ( x[tidx[2*n_f/3:,tq3idx]] - z_d )
    out[vidx] = 1.0/n_f * t_final * alpha * x[vidx]
    out[v2idx] = 1.0/n_f * t_final * alpha * x[v2idx]
    out[vuidx] = 1.0/n_f * t_final * beta * x[vuidx]
    out[v2uidx] = 1.0/n_f * t_final * beta * x[v2uidx]
...
# test_x = np.ones(n_total)
# test_objf = np.zeros( (1,1) )
# test_objg = np.zeros( (n_total,) )
# objf_local(test_objf, test_x)
# objg_local(test_objg, test_x)
# import ipdb; ipdb.set_trace()
def consf(out, x):
    x_u = np.zeros(n_u)
    x_p = np.zeros(n_p)
    ...
    for i in range(n_f):
        ...
        for j in range(n_e1):
            out[e1idx[i,j]] += x_t.dot( lut[t_range[j]].dot( x_u ))
    # Equation 2, velocity stationary
    out[e2idx] = ( n1[v_range,:].dot( x_u ) +
                   n2[v_range,:].dot( x_u ) +
                   n3[v_range,:].dot( x_p ) )
    for j in range(n_e2):
        out[e2idx[j]] += x_u.dot( nuu[v_range[j]].dot( x_u ) )
...
def consg(out, x):
    ...
    for k in range(n_f): # time loop
        ...
        for j in range(n_e1):
            out[ e1idx[k,j], tidx[k,:] ] += lut[t_range[j]].dot( x_u )[t_range]
            # velocity
            tmp_v = lut[t_range[j]].T.dot( x_t )
            out[e1idx[k,j], uidx] = tmp_v[v_range]
            # velocity control vu
            out[e1idx[k,j], vuidx[0]] = np.dot( tmp_v[vbc_point],
                                               np.ones(len(vbc_point)) )
            # velocity control vu2
            out[e1idx[k,j], vu2idx[0]] = np.dot( tmp_v[vbc_point2],
                                                np.ones(len(vbc_point2)) )
            # velocity control v2u
            out[e1idx[k,j], v2uidx[0]] = np.dot( tmp_v[vbc2_point],
                                                 np.ones(len(vbc2_point)) )
            # velocity control v2u2
            out[e1idx[k,j], v2u2idx[0]] = np.dot( tmp_v[vbc2_point2],
    ...
    for l in range(n_e2):
        # velocity
        tmp_v2 = nuu[v_range[l]].dot(x_u) + nuu[v_range[l]].T.dot(x_u)
        out[e2idx[l], uidx] += tmp_v2[v_range]
        # velocity control vu
        out[e2idx[l], vuidx[0]] += np.dot(tmp_v2[vbc_point], np.ones(len(vbc_point)))
        # velocity control vu2
        out[e2idx[l], vu2idx[0]] += np.dot(tmp_v2[vbc_point2], np.ones(len(vbc_point2)))
        # velocity control v2u
        out[e2idx[l], v2uidx[0]] += np.dot(tmp_v2[vbc2_point], np.ones(len(vbc2_point)))
        # velocity control v2u2
        out[e2idx[l], v2u2idx[0]] += np.dot(tmp_v2[vbc2_point2], np.ones(len(vbc2_point2)))
...
def consgpattern():
    ...
    for i in range(n_f):
        ...
        out[ np.ix_( e1idx[i,:], uidx ) ] = 1
        out[ e1idx[i,:], vuidx[0] ] = 1
        out[ e1idx[i,:], vu2idx[0] ] = 1
        out[ e1idx[i,:], v2uidx[0] ] = 1
        out[ e1idx[i,:], v2u2idx[0] ] = 1
...
x_bnd_l[vuidx[0]] = 0.1
x_bnd_h[vuidx[0]] = 0.4
x_bnd_l[vu2idx[0]] = -0.4
x_bnd_h[vu2idx[0]] = 0.0
x_bnd_l[v2uidx[0]] = -0.4
x_bnd_h[v2uidx[0]] = -0.1
x_bnd_l[v2u2idx[0]] = -0.4
...
init_array = np.load("./results/modelNew2/2016-03-02(local09)/results1.npy")
# init_array = np.ones( (n_total,) )
prob.initPoint( init_array )
...
solver.debug = True
tdy = date.today()
drt = './results/modelNew2/'+str(tdy) + part
if not os.path.exists(drt):
    os.makedirs(drt)
        
# solver.printOpts[ "printLevel" ] = 10
# solver.printOpts["printFile"] = (drt + "/optwrp1.txt")
# solver.printOpts["minorPrintLevel"] = 10
# solver.printOpts["printLevel"] = 10
# solver.solveOpts["qpSolver"] = "cg"
